...
class Mailbox {
	...
	/**
	 * Get email header of specific id
	 * @return header(object/string)
	 */

	public function getHeader($mailId){
		$head              = imap_rfc822_parse_headers( imap_fetchheader( $this->getImapStream(), $mailId, FT_UID ) );

		//Other Options for getting Header Info
		//$head              = imap_header( $this->getImapStream(), $mailId, FT_UID  );
		//$head              = imap_headerinfo( $this->getImapStream(), $mailId, FT_UID  );

		//Use when Looking for custom headers
		/*$head              = imap_fetchheader( $this->getImapStream(), $mailId, FT_UID ) ;
		preg_match_all('/([^: ]+): (.+?(?:\r\n\s(?:.+?))*)\r\n/m',$head, $matches);
		$headers = array_combine($matches[1], $matches[2]);
		$head = $headers['ParentID'];*/
